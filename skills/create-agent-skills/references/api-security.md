...
<the_solution>
Use `$CODEX_HOME/scripts/secure-api.sh` - a wrapper that loads credentials internally.

<for_supported_services>
```bash
# ✅ GOOD - No credentials visible
$CODEX_HOME/scripts/secure-api.sh <service> <operation> [args]

# Examples:
$CODEX_HOME/scripts/secure-api.sh facebook list-campaigns
$CODEX_HOME/scripts/secure-api.sh ghl search-contact "email@example.com"
```
</for_supported_services>
...
1. **Add operations to the wrapper** (`$CODEX_HOME/scripts/secure-api.sh`):
...
3. **Add credential placeholders to `$CODEX_HOME/.env`** using profile naming:

```bash
# Check if entries already exist
grep -q "YOURSERVICE_MAIN_API_KEY=" $CODEX_HOME/.env 2>/dev/null || \
  echo -e "\n# Your Service - Main profile\nYOURSERVICE_MAIN_API_KEY=\nYOURSERVICE_MAIN_ACCOUNT_ID=" >> $CODEX_HOME/.env

echo "Added credential placeholders to $CODEX_HOME/.env - user needs to fill them in"
```
...
1. **Check for saved profile:**
   ```bash
   $CODEX_HOME/scripts/profile-state get yourservice
   ```

2. **If no profile saved, discover available profiles:**
   ```bash
   $CODEX_HOME/scripts/list-profiles yourservice
   ```
...
5. **Save user's selection:**
   ```bash
   $CODEX_HOME/scripts/profile-state set yourservice <selected_profile>
   ```
...
7. **Make API call with profile:**
   ```bash
   $CODEX_HOME/scripts/secure-api.sh yourservice:<profile> list-items
   ```
...
```bash
$CODEX_HOME/scripts/secure-api.sh yourservice:<profile> <operation> [args]

# Examples:
$CODEX_HOME/scripts/secure-api.sh yourservice:main list-items
$CODEX_HOME/scripts/secure-api.sh yourservice:main get-item <ITEM_ID>
```
...
Usage:
```bash
echo '{"name":"value"}' | $CODEX_HOME/scripts/secure-api.sh service create-item
```
</post_with_json_body>
...
<credential_storage>
**Location:** `$CODEX_HOME/.env` (global for all skills, accessible from any directory)
...
```bash
set -a
source $CODEX_HOME/.env 2>/dev/null || { echo "Error: $CODEX_HOME/.env not found" >&2; exit 1; }
set +a
```
</credential_storage>
...
3. **Auto-create credential placeholders** - add empty fields to `$CODEX_HOME/.env` immediately when creating the skill
4. **Keep credentials in `$CODEX_HOME/.env`** - one central location, works everywhere
...
```bash
# Check .env exists
ls -la $CODEX_HOME/.env

# Check specific variables (without showing values)
grep -q "YOUR_API_KEY=" $CODEX_HOME/.env && echo "API key configured" || echo "API key missing"
```
</testing>
