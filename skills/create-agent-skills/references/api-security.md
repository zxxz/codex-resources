...
When Codex executes this, the full command with expanded `$API_KEY` appears in the conversation.
</the_problem>

<the_solution>
Use `~/.codex/scripts/secure-api.sh` - a wrapper that loads credentials internally.

<for_supported_services>
```bash
# ✅ GOOD - No credentials visible
~/.codex/scripts/secure-api.sh <service> <operation> [args]

# Examples:
~/.codex/scripts/secure-api.sh facebook list-campaigns
~/.codex/scripts/secure-api.sh ghl search-contact "email@example.com"
```
</for_supported_services>
...
1. **Add operations to the wrapper** (`~/.codex/scripts/secure-api.sh`):
...
3. **Add credential placeholders to `~/.codex/.env`** using profile naming:

```bash
# Check if entries already exist
grep -q "YOURSERVICE_MAIN_API_KEY=" ~/.codex/.env 2>/dev/null || \
  echo -e "\n# Your Service - Main profile\nYOURSERVICE_MAIN_API_KEY=\nYOURSERVICE_MAIN_ACCOUNT_ID=" >> ~/.codex/.env

echo "Added credential placeholders to ~/.codex/.env - user needs to fill them in"
```
...
1. **Check for saved profile:**
   ```bash
   ~/.codex/scripts/profile-state get yourservice
   ```

2. **If no profile saved, discover available profiles:**
   ```bash
   ~/.codex/scripts/list-profiles yourservice
   ```
...
5. **Save user's selection:**
   ```bash
   ~/.codex/scripts/profile-state set yourservice <selected_profile>
   ```
...
7. **Make API call with profile:**
   ```bash
   ~/.codex/scripts/secure-api.sh yourservice:<profile> list-items
   ```
...
```bash
~/.codex/scripts/secure-api.sh yourservice:<profile> <operation> [args]

# Examples:
~/.codex/scripts/secure-api.sh yourservice:main list-items
~/.codex/scripts/secure-api.sh yourservice:main get-item <ITEM_ID>
```
...
Usage:
```bash
echo '{"name":"value"}' | ~/.codex/scripts/secure-api.sh service create-item
```
</post_with_json_body>
...
<credential_storage>
**Location:** `~/.codex/.env` (global for all skills, accessible from any directory)
...
```bash
set -a
source ~/.codex/.env 2>/dev/null || { echo "Error: ~/.codex/.env not found" >&2; exit 1; }
set +a
```
</credential_storage>
...
3. **Auto-create credential placeholders** - add empty fields to `~/.codex/.env` immediately when creating the skill
4. **Keep credentials in `~/.codex/.env`** - one central location, works everywhere
...
grep -q "YOUR_API_KEY=" ~/.codex/.env && echo "API key configured" || echo "API key missing"
```
</testing>
